...
class FilterTest extends JUnitSuite with MustMatchersForJUnit {
  import FilterOps._
  ...
  @Test def negation {
    ...
    negate(f("A <= 1")) must be (equivalentTo(f("A > 1 OR A IS NULL")))
    negate(f("A >= 1")) must be (equivalentTo(f("A < 1 OR A IS NULL")))
    // TODO: negate(f("A > 1 AND B < 2")) must be (equivalentTo(f("A <= 1 OR A IS NULL OR B >= 2 OR B IS NULL")))
    ...
    // TODO: simplify(negate(f("A < 1 AND A <> 2"))) must be (equivalentTo(f("A IS NULL OR A >= 1")))
    // TODO: simplify(negate(negate(f("A < 1 AND A <> 2")))) must be (equivalentTo(f("A < 1")))
  }

  @Test def redundancy {
    import ECQL.{ toFilter => f }
    redundant(f("A = 2"), f("INCLUDE")) must be (true)
    redundant(f("A = 2"), f("A >= 1")) must be (true)
    redundant(f("A > 1"), f("A = 1")) must be (false)
    redundant(f("A = 1"), f("A > 1")) must be (false)
    redundant(f("A < 1"), f("A < 1")) must be (true)
    redundant(f("A < 2"), f("A < 1")) must be (false)
    redundant(f("A < 1"), f("A < 2")) must be (true)
    redundant(f("A < 1 OR B > 2"), f("A < 1")) must be (false)
    redundant(f("A < 0 OR B > 2"), f("A < 1")) must be (false)
    redundant(f("A < 1"), f("A IS NOT NULL")) must be (true)
    isSubSet(f("A <> 2"), f("A < 2")) must be (true)
    redundant(f("A < 1"), f("A <> 2")) must be (true)
    redundant(f("A < 1 AND A <> 2"), f("A < 1")) must be (true)
    isSubSet(f("A < 1"), f("A < 1 AND A > 0")) must be (true)
    redundant(f("A < 1 AND A > 0"), f("A < 1")) must be (true)
    redundant(f("A < 1"), f("A < 1 AND A > 0")) must be (false)
    isSubSet(f("A <> 1"), f("A < 1")) must be (true)
    isSubSet(f("A <> 1 OR A IS NULL"), f("A IS NULL")) must be (true)
    isSubSet(f("A = 1 OR A = 2"), f("A = 1 OR A = 2")) must be (true)
    isSubSet(f("A <> 1 OR A IS NULL"), f("A < 1 OR A IS NULL")) must be (true)
  }

  @Test def binopSubsets {
    import ECQL.{ toFilter => f }

    isSubSet(f("A = 1"), f("A = 1")) must be (true)

    isSubSet(f("A = 1"), f("A <> 1")) must be (false)
    ...
    isSubSet(f("A <> 1"), f("A = 1")) must be (false)
    isSubSet(f("A <> 1"), f("A = 2")) must be (true)

    isSubSet(f("A <> 1"), f("A <> 1")) must be (true)

    isSubSet(f("A <> 1"), f("A < 1")) must be (true)
    isSubSet(f("A <> 1"), f("A < 0")) must be (true)
    isSubSet(f("A <> 1"), f("A < 2")) must be (false)
    ...
    isSubSet(f("A <> 1"), f("A > 1")) must be (true)
    isSubSet(f("A <> 1"), f("A > 0")) must be (false)
    isSubSet(f("A <> 1"), f("A > 2")) must be (true)
    ...
    isSubSet(f("A IS NULL"), f("A IS NULL")) must be (true)
    isSubSet(f("A IS NULL"), f("A IS NOT NULL")) must be (false)
    isSubSet(f("A IS NOT NULL"), f("A IS NULL")) must be (false)
    isSubSet(f("A IS NOT NULL"), f("A = 1")) must be (true)
    isDisjoint(f("A IS NOT NULL"), f("A <> 1")) must be (false)
    isSubSet(f("A IS NOT NULL"), f("A <> 1")) must be (true)
  }

  @Test def constrainFilters {
    import ECQL.{ toFilter => f }
    constrain(f("A < 1 OR A IS NULL"), f("A IS NOT NULL")) must be (equivalentTo(f("A < 1")))
    ...
    constrain(f("A <= 1"), f("A < 1")) must be (equivalentTo(f("A < 1")))
    intersection(f("A LIKE 'abc%'"), f("A NOT LIKE 'abc%'")) must 
      be (Some(Filter.EXCLUDE))
    constrain(f("A > 2 AND A < 4"), f("A > 4")) must be (equivalentTo(f("EXCLUDE")))
    constrain(f("A IS NULL"), f("A IS NOT NULL")) must be (equivalentTo(f("EXCLUDE")))
    constrain(f("A > 2"), f("A IS NOT NULL")) must be (equivalentTo(f("A > 2")))
    constrain(f("A IS NOT NULL"), f("A > 2")) must be (equivalentTo(f("A > 2")))
    constrain(f("A > 2"), f("A IS NULL OR A < 10")) must 
      be (equivalentTo(f("A > 2 AND A < 10")))
  ...
  @Test def simplification {
    ...
    simplify(f("A <> 2 AND A IS NOT NULL")) must be (equivalentTo(f("A <> 2")))
    simplify(f("(A <> 2 OR A IS NULL) AND A IS NOT NULL")) must be (equivalentTo(f("A <> 2")))
    simplify(f("NOT A < 1")) must be (equivalentTo(f("A >= 1 OR A IS NULL")))
    ...
    simplify(f("A > 15 AND (A < 20 OR A IS NULL)")) must 
      be (equivalentTo(f("A > 15 AND A < 20")))
  }
}
