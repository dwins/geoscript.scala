...
// externalGraphic, well-known graphic , color
   ...
   * given Rule.
   */
  def symbolize(rule: SimpleRule): List[Pair[Double, Symbolizer]] = {
  ...
  case class SimpleRule(
    ...
    def merge(that: SimpleRule): SimpleRule =
      SimpleRule(
        Description.combine(this.description, that.description),
        simplify(this.selectors ++ that.selectors),
  ...
  val EmptyRule = SimpleRule(Description.Empty, List(AcceptSelector), Nil)
  def specificityOrdering(a: SimpleRule, b: SimpleRule): Boolean =
  ...
  def css2sld(styleSheet: List[Rule]): gt.Style = {
    ...
    val rules =
      stableSort(assortedRules, specificityOrdering _).toList.reverse
   ...
   * discovered.
   */
   def combinations(xs: Seq[SimpleRule])(prune: SimpleRule => Boolean)
   : Seq[SimpleRule] =
     xs match {
